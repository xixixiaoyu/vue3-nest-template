...
## 更新摘要
**已做更改**   
- 更新了“Zod Schema 与类型推断分析”部分，详细说明 `passwordSchema` 的标准化定义及其在 `ResetPasswordView` 等组件中的复用
- 强调了 `passwordSchema` 作为基础密码规则在多个 Schema（如 `RegisterSchema` 和 `ResetPasswordSchema`）中的统一应用
- 更新了相关源码引用，新增对 `ResetPasswordView.vue` 的引用以反映前端验证逻辑的变更
...
## 简介
`shared` 包是本项目的核心基础设施，作为前后端共享类型和验证逻辑的“单一可信源”。它通过 Zod Schema 统一定义数据结构和验证规则，确保前后端在数据验证、类型定义和接口契约上完全一致，从而显著减少因类型不匹配或验证逻辑差异导致的错误。该包不仅包含用于表单和 API 请求/响应的 Zod Schema，还导出了通用的 DTO（如 `ApiResponse`）和实用工具函数（如 `formatUser`），为整个应用提供一致的基础支持。近期引入了标准化的 `passwordSchema`，被 `ResetPasswordView` 等组件广泛使用，替代了原有的自定义验证逻辑，进一步增强了验证规则的一致性。
...
RPV[ResetPasswordView.vue] --> |使用| ZS
end
ZS --> |被引用| BD
ZS --> |被引用| LF
ZS --> |被引用| RPV
...
#### Schema 定义与复用
该文件首先定义了可复用的基础字段规则，如 `emailSchema` 和 `passwordSchema`。这些基础规则随后被组合成完整的对象 Schema，例如 `LoginSchema`、`RegisterSchema` 和 `ResetPasswordSchema`。这种分层设计避免了重复代码，确保了验证逻辑的一致性。特别是 `passwordSchema`，现在作为标准化的密码验证规则，被多个 Schema 复用，包括 `RegisterSchema` 和 `ResetPasswordSchema`，确保了密码规则在整个应用中的一致性。
...
ResetPasswordSchema --> passwordSchema : "使用"
```

**Diagram sources**
- [auth.schema.ts](file://packages/shared/src/schemas/auth.schema.ts#L6-L42)
- [auth.schema.ts](file://packages/shared/src/schemas/auth.schema.ts#L96-L101)

#### 自动类型推断
文件末尾使用 `z.infer<typeof Schema>` 语法，从每个 Zod Schema 自动推断出对应的 TypeScript 类型。例如，`LoginInput` 类型就是从 `LoginSchema` 推断出来的。这消除了手动编写 `interface LoginInput { email: string; password: string }` 的需要，保证了类型定义与验证规则的绝对同步。`ResetPasswordInput` 类型也从 `ResetPasswordSchema` 推断而来，确保了前后端对重置密码请求结构的一致理解。

**Section sources**
- [auth.schema.ts](file://packages/shared/src/schemas/auth.schema.ts#L88-L93)
- [auth.schema.ts](file://packages/shared/src/schemas/auth.schema.ts#L112-L112)
...
## 结论
`shared` 包是本全栈应用成功的关键设计。它通过 Zod Schema 实现了“单一可信源”，将数据验证和类型定义从分散的状态集中管理，极大地提升了代码的一致性、可维护性和安全性。前后端共同引用同一套定义，不仅减少了重复工作，更从根本上杜绝了因类型不一致导致的运行时错误。结合通用 DTO 和工具函数，`shared` 包为整个项目提供了一个坚实、可靠且高效的基础。引入标准化的 `passwordSchema` 并在 `ResetPasswordView` 等组件中复用，是这一设计理念的又一次成功实践，确保了密码验证规则在整个应用生命周期中的一致性和可维护性。