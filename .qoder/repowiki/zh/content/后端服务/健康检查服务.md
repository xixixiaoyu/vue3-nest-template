...
- HealthController 暴露的 /health、/health/liveness、/health/readiness 端点及响应语义
...
```mermaid
graph TB
HC["HealthController<br/>/health, /health/liveness, /health/readiness"] --> HCS["HealthCheckService"]
...
- [apps/backend/Dockerfile](file://apps/backend/Dockerfile#L75-L78)
- [docker-compose.yml](file://docker-compose.yml#L69-L75)
- [apps/frontend/nginx.conf](file://apps/frontend/nginx.conf#L20-L25)
...
- /health：综合健康检查，包含数据库、Redis、内存堆与 RSS、磁盘使用率等维度。
- /health/liveness：简单存活探针，返回固定结构，便于容器运行时快速判定进程存活。
- /health/readiness：就绪探针，仅检查数据库与 Redis，确保业务流量只在关键依赖可用时进入。
...
- 分层探测：将 /health 作为周期性探测端点，将 /health/readiness 用于就绪判断，避免在热路径上频繁全量检查。

[本节为通用建议，不直接分析具体文件]

## 故障排查指南
- 数据库不可达
  - 现象：/health 或 /health/readiness 返回数据库不健康。
  - 排查：确认 DATABASE_URL、网络连通性、PostgreSQL 服务状态；查看 PrismaHealthIndicator 抛出的错误消息。
- Redis 不可用
  - 现象：/health 或 /health/readiness 返回 Redis 不健康。
  - 排查：确认 REDIS_HOST/PORT/PASSWORD/DB 等配置；查看 RedisModule 的连接日志与重试策略；验证短 TTL 测试键是否可写可读。
- 内存/磁盘阈值告警
  - 现象：/health 中内存或磁盘指标触发不健康。
...
### 在 Docker 容器中的应用
- 容器镜像内置 HEALTHCHECK，使用 wget 访问 /health，失败即退出容器重启。
...
- 存活探针（livenessProbe）：建议使用 /health/liveness，返回固定结构，快速判定进程存活。
- 就绪探针（readinessProbe）：建议使用 /health/readiness，仅检查数据库与 Redis，避免在依赖未就绪时接收流量。
- 建议参数示例（请根据实际环境调整）：
...
### 响应缓存策略（建议）
- 对于 /health：若需要降低对数据库与 Redis 的压力，可在应用层对健康状态进行短期缓存（例如 10-30 秒），异常时立即失效。
- 对于 /health/liveness：由于返回固定结构，通常无需缓存。
- 对于 /health/readiness：建议保持实时性，避免误判依赖恢复时机。
