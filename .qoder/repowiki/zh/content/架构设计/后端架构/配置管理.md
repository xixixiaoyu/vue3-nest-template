...
## 更新摘要
**变更内容**   
- 新增开发环境专用的 `docker-compose.dev.yml` 文件说明，包含 PostgreSQL 和 Redis 服务的完整配置
- 更新容器化部署配置部分，详细说明开发环境热重载机制与生产环境的差异
- 扩展环境变量管理部分，补充开发环境与生产环境配置优先级说明
- 更新异步配置注入部分，增加对开发环境配置动态加载的说明
- 补充开发环境与生产环境的 Dockerfile 构建策略差异
