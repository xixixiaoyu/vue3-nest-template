...
## 更新摘要
**变更内容**   
- 更新了环境变量管理部分，新增 Redis、邮件、S3 存储、速率限制等配置说明
- 更新 JWT 过期时间配置说明，区分访问令牌和刷新令牌
- 扩展异步配置注入部分，涵盖邮件模块和文件上传模块的配置注入
- 更新容器化部署配置，补充 S3 和邮件相关环境变量注入
- 新增配置错误排查的常见场景
...
在 `docker-compose.yml` 中，后端服务的环境变量配置已扩展：
